...
from server_types import Invite, Group, Message, User
...
reserved_names = ['system', 'global', 'admin', 'null', 'none']
...
def handle_client(socket: sockets.socket, full_address: str):
	...
	while True:
		uname = Message.read_from_stream(input_stream, server_state_lock, users, groups).content.strip()
		set_uname = True
		server_state_lock.acquire_read()
		try:
			for user in users:
				if user.name == uname:
			...
			server_state_lock.release_read()
		if not set_uname:
			continue
		this_user.name = uname
		break

	server_state_lock.acquire_write()
	users.append(this_user)
	global_group.users.append(this_user)
	server_state_lock.release_write()
	global_group.send_system_message(f"{this_user.name} has connected")

	try:
		while True:
			message = Message.read_from_stream(input_stream, server_state_lock, users, groups)
			if message.content.startswith('/create '):
				...
				# forward to target(s)
				message.target.send_bytes_async(message.to_bytes())
	...
	try:
		while True:
			...
			try:
				...
				c_thread = threading.Thread(target=handle_client, args=(client_socket, address[0] + ':' + str(address[1])))
